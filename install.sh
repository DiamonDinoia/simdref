...
if command -v uv >/dev/null 2>&1; then
	...
	uv venv "$VENV_DIR"
	# Build isolation stays on: a fresh `uv venv` carries no setuptools.
...
"$VENV_DIR/bin/python" -m simdref update
...
Manpages render on demand from the catalog DB. To pre-generate man7 pages
for \`man -M\` integration (~150k files): simdref install-manpages
EOF
