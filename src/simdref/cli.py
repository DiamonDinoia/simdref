...
def show(
    mnemonic: str = typer.Argument(..., help="Instruction mnemonic (e.g. vgatherdpd, vfmadd213pd)."),
    arch: str | None = typer.Option(None, "--arch", help="Pin output to a specific microarch (e.g. sapphirerapids, zen4, skylake-x)."),
    include_modeled: bool = typer.Option(False, "--include-modeled", help="Fall back to modeled perf data when no measured data exists."),
    output_json: bool = typer.Option(False, "--json", help="Emit a structured JSON record with lat/cpi/ports per arch instead of text."),
) -> None:
    """Print docs and per-arch lat/cpi for an instruction mnemonic.

    Non-interactive stdout-only command — does not open the TUI. Accepts
    the same ``--arch`` aliases as ``annotate`` (resolves ``sapphirerapids``
    to ``EMR`` and so on). Exits 2 when no record matches.
    """
    ...
    records = _find_instructions_fast(mnemonic)
    if not records:
        err_console.print(f"no instruction match for {mnemonic!r}", style="yellow")
    ...
    if output_json:
        ...
        for rec in records:
            ...
            summary = aggregate_perf(rec, mode="avg", include_modeled=include_modeled)
    ...
    typer.echo(f"# {mnemonic}  ({len(records)} variant{'s' if len(records) != 1 else ''})")
    for rec in records:
        typer.echo("")
        typer.echo(f"[{rec.key}]")
        if rec.summary:
