...
simdref source tree, which is also published as a Claude Code plugin
marketplace. Preferred install (one-liner at the Claude Code prompt):
...
Manual alternatives (symlink from a checkout for always-current, or a
one-off snapshot):
```bash
